...
function filterOutSavedDramas(
  pending: ExtractedDrama[],
  ...
  return pending.filter((item) => !savedIds.has(item.id))
}
...
function tryShowPendingDialogOnceOnVisit(): void {
  ...
  chrome.storage.local.get([PENDING_DRAMAS_KEY, SAVED_DRAMAS_KEY], (result) => {
    const rawPending = (result[PENDING_DRAMAS_KEY] as ExtractedDrama[] | undefined) || []
    const saved = (result[SAVED_DRAMAS_KEY] as ExtractedDrama[] | undefined) || []
    const filteredPending = filterOutSavedDramas(rawPending, saved)

    mergeDramasFromList(filteredPending)
    const toShow = Array.from(dramasToSave.values())

    if (toShow.length === 0) {
      if (rawPending.length > 0) {
        chrome.storage.local.remove(PENDING_DRAMAS_KEY)
        console.log('[Content] 待处理列表无需提示，已清空')
      }
      return
    }

    if (toShow.length !== rawPending.length) {
      saveDramasToStorage()
      console.log('[Content] 已从待处理列表移除无需提示项并写回存储')
    }
